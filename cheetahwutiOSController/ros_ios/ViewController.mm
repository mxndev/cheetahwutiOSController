...
- (BOOL)shouldPerformSegueWithIdentifier:(NSString *)identifier sender:(id)sender
{
    if([ViewController isValidIp:[ip_text_field.attributedText string]])
    {
        ...
        {
            NSLog(@"ROS jest zainicjowany. Nie można zmienić ROS_MASTER_URI");
        }
        
        if(ros::master::check())
        {
            NSLog(@"Połączono do ROS Mastera!");
        }
        else
        {
            UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"Błąd!" message:@"Nie można połączyć do ROS Mastera." delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil];
            [alert show];
            return NO;
        }
    }
    ...
    {
        UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"Błąd!" message:@"Adres IP ROS Mastera jest niepoprawnym typem IPv4." delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil];
        [alert show];
        return NO;
    }
    return YES;
